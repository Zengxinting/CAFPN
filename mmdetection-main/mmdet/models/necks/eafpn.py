...
class CAFPN(nn.Module):
    ...
    def __init__(self, in_channels):
        ...
        # 定义DCE模块
        # ------------------------------- #
        self.SCE = DCE(in_channels=in_channels)

        # ------------------------------- #
        # 定义CSAG模块
        # ------------------------------- #
        self.CAG = CSAG(in_channels=in_channels // 8)
        ...
        # 经过SSF后的1x1卷积
        self.SSF_C5 = nn.Conv2d(1024, 256, 1)
        self.SSF_C4 = nn.Conv2d(512, 256, 1)
        ...
        self.conv_1x1 = nn.Conv2d(768, 256, 1)
        for m in self.modules():
            if isinstance(m, nn.Conv2d):
                xavier_init(m, distribution='uniform')
    ...
    def forward(self, x):
        ...
        R5 = (R5 + F.adaptive_max_pool2d(SA, output_size=C5.shape[-2:])) * CA
        residual_R4 = F.adaptive_max_pool2d(I, output_size=C4.shape[-2:])
        R4 = (residual_R4 + F.adaptive_max_pool2d(SA, output_size=C4.shape[-2:])) * CA
        residual_R3 = F.interpolate(I, size=C3.shape[-2:], mode="nearest")
        R3 = (residual_R3 + F.interpolate(SA, size=C3.shape[-2:], mode="nearest")) * CA
        residual_R2 = F.interpolate(I, size=C2.shape[-2:], mode="nearest")
        R2 = (residual_R2 + F.interpolate(SA, size=C2.shape[-2:], mode="nearest")) * CA
...
if __name__ == "__main__":
    ela = CAFPN(in_channels=2048)

    # 创建一个有序字典作为输入
    input1 = torch.rand(3, 256, 64, 64)
    input2 = torch.rand(3, 512, 32,32)
    input3 = torch.rand(3, 1024, 16,16)
    input4 = torch.rand(3, 2048, 8,8)

    # 前向传播
    output =  ela([input1, input2, input3, input4])
    # 打印出输出张量的形状
